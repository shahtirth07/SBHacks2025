...
Original file is located at
    https://colab.research.google.com/drive/1BQsAfzTQM80dVB5AmuKbVGkcag3o_mGd
"""
...
# Specify Tesseract path for Colab
...
output_text_dir = os.path.join(image_dir, "extracted_text")
...
extract_text_from_images(image_dir, output_text_dir)
    ...
    for file in os.listdir(output_dir):
        if file.endswith(".png") or file.endswith(".csv"):
            ...
            if file.endswith(".png"):
                ...
                # Remove binary_representation to avoid metadata size issues
                # Images are NOT included in metadata directly
                ...
                if os.path.exists(caption_path):
                    with open(caption_path, "r") as f:
                        metadata["text_representation"] = f.read()
            # Generate embedding
            embedding = get_embedding(metadata.get("text_representation", ""))
            if embedding:
                # Upload to Pinecone
...
if __name__ == "__main__":
    # Upload extracted data to Pinecone
    output_dir = "data"  # Path to extracted images, tables, and captions
    upload_to_pinecone(output_dir)

    # Ask Claude with a user question
...
pip install pinecone-client pytesseract pillow pandas

# Inspect metadata for each match
result = index.query(vector=[0] * 1536, top_k=10, include_metadata=True)
for item in result["matches"]:
    print(f"Metadata for match {item['id']}: {item['metadata']}")
